 ...
 * a response or an showAll was received.
 *
 ...
 *   if (result) {
 *     // parse the result as a JSON object and process it as an incoming showAll or the answer to a previously sent request
 *   }
 * }
 * \endcode
 */
 ...
 * \return JSON-serialized null-terminated incoming showAll or request response. May be NULL if the timeout expires.
 */
 ...
 *   if (result) {
 *     // parse the result as JSON object and process it as an incoming showAll or an answer to a previously sent request
 *   }
 * }
